...
{
    for( int i = 0; i < agents_total; i++ )
	printf("id: %f status: %f x: %f y: %f z: %f, w: %f \n", h_agents_in.ids[i].x, h_agents_in.ids[i].y, h_agents_in.pos[i].x, h_agents_in.pos[i].y, h_agents_in.pos[i].z, h_agents_in.pos[i].w);
}
...
bool initGL(int *argc, char **argv)
{
    ...
    gluPerspective(60.0, (GLfloat)window_width / (GLfloat) window_height, 0.1, 1024.0);  //HPV se cambio de 10 a 1024 el ultimo campo ZFar
    glTranslated(-256, -256, -512); //HPV Se cambia la longitud de alcance de vision con ZFar y tmb el sistema de coordenadas
...
void display()
{
    ...
    // render from the vbo
    glBindBuffer(GL_ARRAY_BUFFER, vbo);
    //glVertexPointer(4, GL_FLOAT, 0, 0); --original se cambio a 2 para que solo tomara x e y
