...
void compute_process()
{
	...
	for(int it = 0; it < nreps ; it++)
	{
		...
		err = cudaMemcpy(h_agents_pos, d_agents_pos, num_bytes_agents, cudaMemcpyDeviceToHost);
		if (err != cudaSuccess)
	    	{
		...
		err = cudaMemcpy(h_agents_ids, d_agents_ids, num_bytes_agents, cudaMemcpyDeviceToHost);
		if (err != cudaSuccess)
	    	{
	        	fprintf(stderr, "Failed to copy pointer from device to host (error code %s)!\n", cudaGetErrorString(err));
	        	exit(EXIT_FAILURE);
